	...
	<div class="row">
      ...
      @endif --}}
            ...
            @foreach ($post as $posts)
				...
				<form action="{{ url('/post/edit/') }}" method="post" class="" enctype="multipart/form-data">
					{{ csrf_field()}}
					<div class="form-group has-feedback">
						<label for="" class="text mt-1" style="font-size: 20px;">Judul Artikel</label>
						<input type="text" class="form-control inp" name="title" placeholder="Judul Artikel" value="{{ $posts->judul}}" required>
					</div>
					<div class="form-group has-feedback">
						<label for="" class="tex" style="font-size: 20px;">Konten Artikel</label>
						<textarea class="form-control inp" name="content" id='article-ckeditor' cols="" rows="5" {{-- placeholder="Konten Artikel"  --}} required>{{ $posts->isi_artikel }}</textarea>
